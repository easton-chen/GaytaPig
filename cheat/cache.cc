...
void Cache::HandleRequest(unsigned long addr, unsigned int bytes, int read,
  ...
  if (!BypassDecision()) {
    ...
    for(unsigned long i=first;i<=last;++i)
    {
      ...
      {
          #ifdef DEBUG
          printf(" # Cache %c HIT # vpn = 0x%llx, read = %d \n",name,i,read);
          #endif
          if(read==1)
          {
          ...
          else if(read==0)
          {
            ...
            else if(config_.write_through==0)
            {
          ...
          //flags的更新
            ...
            for(;k<(set+1)*config_.associativity;++k)
            {
      ...
      Load(i,hit,cycle);//只负责将条目更新，然后让这个查询再来一次。Load里面对响应的miss以及底层的hit和cycle做了更新。
      //cycle和hit的操作
      cycle-=config_.AccessTime;
      --i;
...
void Cache::Load(unsigned long vpn,int &hit,int &cycle){
  ...
  unsigned int set=get_set(vpn,config_.set_num);
  unsigned int index=set*config_.associativity;
  char *tmp_block=new char[entity_.block_size];
  ...
  for(;index<(set+1)*config_.associativity;++index)
  {
    ...
    if(entity_.content[index].flags>max)
    {
