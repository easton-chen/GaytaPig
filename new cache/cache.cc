...
void Cache::HandleRequest(uint64_t addr, int byte, int read,
  ...
  if (ReplaceDecision(addr, pre_flag)) {
    ...
    if (read == 0) {
      ...
      if(cc.write_through == 0) { //write back
        ...
        lower_->HandleRequest(addr, byte, read, content,
    ...
    if(cc.algorithm_sort == 2) {
      ...
      sets[set_index].update_LIRS(tag);
    }
...
int Cache::BypassDecision(uint64_t addr) {
  CacheConfig cc;
  GetConfig(cc);
  if(cc.bypass_switch == 0)   return false;
  int t=0,s=0,b=0;
  int block_size = cc.block_size;
  int set_num = cc.set_num;
  while(block_size != 1){
...
//case 1:find a valid block  case 2:conflict miss
void Cache::ReplaceAlgorithm(uint64_t addr, int read, int &time, bool pre_flag) {
    ...
    if(!sets[set_index].line[replace_index].valid) {
      ...
      //case 2: conflict miss, need replacement
      stats_.replace_num++;
...
int Cache::PrefetchDecision() {
  CacheConfig cc;
  GetConfig(cc);
  return cc.prefetch_flag == 1;
}

void Cache::PrefetchAlgorithm(uint64_t addr) {
  CacheConfig cc;
  GetConfig(cc);
  uint64_t prefetch_addr[3];
  for(int i = 0; i < 3; i++) {
...
void Cache::insert(uint64_t addr)
{
  ...
  if(!sets[set_index].line[replace_index].valid) {
    //case 1:find a valid block
    sets[set_index].line[replace_index].timestamp = request_num/2;
    sets[set_index].line[replace_index].valid = true;
    sets[set_index].line[replace_index].dirty = false;
    sets[set_index].line[replace_index].tag = tag;
  }
