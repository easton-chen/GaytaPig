...
- L1 Cache： Miss Rate = （ 0.199851 ）

- L2 Cache： Miss Rate = （ 0.366446 ）
...
- L1 Cache： Miss Rate = （ 0.113404 ）

- L2 Cache： Miss Rate = （ 0.750450 ）
...
3. 请填写最终确定的优化方案，并陈述理由。对于涉及到的算法，需要详细描述算法设计和实现思路，并给出优缺点分析。（40分）
    ...
    **LFU：**
    ...
    ​	LFU的思想是，考虑某一个cache block，如果这个block被频繁访问（被访问的频率很高），那么我们倾向于认为这个block在后续也会有很大的概率被访问，反过来讲，一个块相对于其他的块被访问的频率较低，那么发生替换时，就把这个块从cache中移除，这就是LFU的思想。

    ​	LFU的实现比较简单，为每一个被访问过的cache block维护一个访问频率（frequency），每一次访问这个cache block都会使他的frequency增加，当需要替换的时候，就直接找出frequency最小的替换出去即可。这里有一个值得提到的点是，在预取的时候，预取不应该算作是一次访问，所以预取的块的frequency是不能更新的，后续的LRU预取时不更新也是同样的道理。

    ​	LFU的优点是简洁明白，而且相比LRU考虑了块被访问的频次，但是缺点也是比较明显的，frequency不能体现出某一个时间段内的访问倾向性，比方说，某一个块在最开始的时候被非常频繁的访问以至于其frequency异常的大，随后不再访问它，但由于他前面“积累”了很多的frequency，所以很难被移除出cache，这就造成了空间上的浪费，白白占用了这一个cache block。

    ​

    **LRU：**

    ​	注：本次lab最终没有采用LRU的替换策略，这里仅仅是叙述下其思想和实现。
...
- L1 Cache： Miss Rate = （ 0.169384 ）

- L2 Cache： Miss Rate = （  0.250581  ）
...
- L1 Cache： Miss Rate = （  0.028374  ）

- L2 Cache： Miss Rate = （  0.434213  ）
